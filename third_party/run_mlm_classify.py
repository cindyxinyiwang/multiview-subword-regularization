...
def train(args, train_dataset, model, tokenizer, lang2id=None):
  ...
  trained_params = []
  params = []
  meta_name = ["embedding", "layer.0"]
  for n, p in model.named_parameters():
    trained_params.append((n, p))
    if any(nd in n for nd in meta_name):
        params.append(p)
  ...
  for _ in train_iterator:
    ...
    for step, batch in enumerate(epoch_iterator):
      ...
      if args.mlm_loss_action == "lmask":
        mlm_inputs["reduction"] = 'none'
        mlm_outputs = model.forward_mlm(**mlm_inputs)
        mlm_loss = mlm_outputs[0]

        z = torch.ones_like(mlm_loss).requires_grad_(True)
        mlm_grad = torch.autograd.grad(mlm_loss, params, grad_outputs=z, create_graph=True, retain_graph=True)
        ce_grad = torch.autograd.grad(loss, params, retain_graph=True)
        jvp_loss = 0 
        for mlm_g, ce_g in zip(mlm_grad, ce_grad):
            jvp_loss += (mlm_g*ce_g).sum()
        target_mlm_mask = ~target_mlm.to(args.device).eq(-100).view(-1)
        dot_prod = torch.autograd.grad(jvp_loss, [z], retain_graph=False, only_inputs=True)[0]
        loss_mask = ((dot_prod >= 0) & target_mlm_mask).float()

        mlm_loss = (mlm_loss*loss_mask).sum()/loss_mask.sum()
...
def main():
  ...
  parser.add_argument("--mlm_loss_action", type=str, default="", help="For distant debugging.")
