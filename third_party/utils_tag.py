...
def create_sde_pretrain_vocab_features(max_seq_length,
                 tokenizer,
                 max_ngram_size=20):
  ...
  pad_token_label_id = pad_token
  pad_token_segment_id = pad_token
  features = []
  for vocab_start in range(100, len(vocab_words), max_seq_length):
    vocab_end = min(len(vocab), vocab_start+max_seq_length)
    vocab_example = vocab_words[vocab_start:vocab_end]

    sde_ids, subword_ids = [], []
    sde_tokens = []
    subword_tokens = []
    for v in vocab_example:
        subword_ids.append(vocab.get(v))
        sde_pieces = tokenizer.ngram_tokenize(v)
        sde_tokens.append(sde_pieces)
        subword_tokens.append(v)
        cur_sde_ids = tokenizer.convert_tokens_to_sde_ids(sde_pieces, max_ngram_size)
        sde_ids.extend(cur_sde_ids)
    input_ids = sde_ids
    label_ids = subword_ids
    segment_ids = [0] * len(subword_tokens)
    ...
    if vocab_start < 5:
      logger.info("*** Example ***")
      logger.info("subword_tokens: %s", " ".join([str(x) for x in subword_tokens]))
      logger.info("sde_tokens: %s", " ".join([str(x) for x in sde_tokens]))
