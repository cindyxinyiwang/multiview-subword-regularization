...
for BPE_DROP in 0.2; do
  for KL in 0.6; do
  for SEED in 5;
  do
    MODEL_PATH=$OUT_DIR/$SRC/${MODEL}_LR${LR}_EPOCH${NUM_EPOCHS}_maxlen${MAXL}_mbped${BPE_DROP}_kl${KL}_klt${KL_T}_kltb${KL_TB}_kltg${KL_TG}_klsg${KL_SG}_resample${RESAMPLE}_s${SEED}
    mkdir -p $MODEL_PATH
    ...
    CUDA_VISIBLE_DEVICES=$GPU python third_party/run_mv_squad.py \
